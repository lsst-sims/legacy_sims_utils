...
def photo_m5(visitFilter, musky, FWHMeff, expTime, airmass, tauCloud=0):
    ...
    if not hasattr(photo_m5, 'Cm'):
        photo_m5.Cm = {'u': 22.74,
                       ...
                       'z': 24.15,
                       'y': 23.70}
        photo_m5.dCm_infinity = {'u': 0.75,
                                 'g': 0.19,
                                 'r': 0.10,
                                 'i': 0.07,
                                 'z': 0.05,
                                 'y': 0.04}
        photo_m5.kAtm = {'u': 0.50,
                         'g': 0.21,
                         'r': 0.13,
                         'i': 0.10,
                         'z': 0.07,
                         'y': 0.18}
        photo_m5.msky = {'u': 22.95,
                         'g': 22.24,
                         'r': 21.20,
                         'i': 20.47,
                         'z': 19.60,
    ...
    Tscale = expTime / 30.0 * np.power(10.0, -0.4*(musky - photo_m5.msky[visitFilter]))
    dCm = 0.
    dCm += photo_m5.dCm_infinity[visitFilter]
    dCm -= 1.25*np.log10(1 + (10**(0.8*photo_m5.dCm_infinity[visitFilter]) - 1)/Tscale)
    # Calculate fiducial m5
    m5 = (photo_m5.Cm[visitFilter] + dCm + 0.50*(musky-21.0) + 2.5*np.log10(0.7/FWHMeff) +
          1.25*np.log10(expTime/30.0) - photo_m5.kAtm[visitFilter]*(airmass-1.0) - 1.1*tauCloud)
